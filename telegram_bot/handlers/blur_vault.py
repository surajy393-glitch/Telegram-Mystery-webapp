...
async def cmd_vault(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.info(f"[cmd_vault] User {uid} accessed /vault command")
        ...
        if uid != 647778438 and not reg.has_active_premium(uid):
            ...
            log.info(f"[cmd_vault] User {uid} is not premium, showing lock screen")
            await update.message.reply_text(LOCK_TEXT, reply_markup=_upgrade_kb(), parse_mode="Markdown")
            return

        # PREMIUM: Show vault browser
        log.info(f"[cmd_vault] User {uid} has premium access, showing vault menu")
        await show_vault_main_menu(uid, update.message.reply_text)
    except Exception as e:
        log.error(f"[cmd_vault] Error for user {uid}: {e}", exc_info=True)
        await update.message.reply_text("❌ Error loading vault. Please try again later.")
...
def register(app):
    ...
    log.info("[blur_vault] Registering vault handlers...")
    ...
    log.info("[blur_vault] ✅ Vault handlers registered successfully (including /vault command at group=-1)")